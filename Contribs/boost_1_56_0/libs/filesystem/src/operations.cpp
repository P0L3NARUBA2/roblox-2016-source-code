...
#     define WINVER 0x501
#   endif
#   if defined(RBX_PLATFORM_UWP) && !defined(WINAPI_FAMILY)
...
# endif
...
#ifdef RBX_PLATFORM_WIN_DURANGO
  #undef BOOST_COPY_DIRECTORY
  #define BOOST_COPY_DIRECTORY(F,T) \
    ((::CreateDirectoryW(T, 0) != 0) || (::GetLastError() == ERROR_ALREADY_EXISTS))

  #undef BOOST_COPY_FILE
  #define BOOST_COPY_FILE(F,T,FailIfExistsBool) copy_file_api_durango(F, T, FailIfExistsBool)
  ...
  #undef BOOST_COPY_FILE
  #define BOOST_COPY_FILE(F,T,FailIfExistsBool) copy_file_api_uwp(F, T, FailIfExistsBool)
  
  #undef BOOST_CREATE_HARD_LINK
  #define BOOST_CREATE_HARD_LINK(F,T) (false)
  #undef BOOST_CREATE_SYMBOLIC_LINK
  #define BOOST_CREATE_SYMBOLIC_LINK(F,T,Flag) (false)
#endif
...
#   endif
...
#if !defined(RBX_PLATFORM_WIN_DURANGO) && !defined(WINAPI_FAMILY_APP)
...
#else
  bool is_reparse_point_a_symlink(const path&)
  {
    return false;
  }
#endif
...
#ifndef RBX_PLATFORM_WIN_DURANGO
  ...
  BOOL resize_file_api(const wchar_t* p, boost::uintmax_t size)
  {
    ...
    HANDLE handle = CreateFile2(p, GENERIC_WRITE, 0, OPEN_EXISTING, &params);
#endif
    LARGE_INTEGER sz;
    sz.QuadPart = size;
...
#else
  PtrCreateHardLinkW create_hard_link_api = nullptr;
#endif
...
#else
  PtrCreateSymbolicLinkW create_symbolic_link_api = nullptr;
#endif
  ...
  bool copy_file_api_durango(const wchar_t* from_p, const wchar_t* to_p, bool fail_if_exists)
  {
    const DWORD buf_sz = 32768;
    boost::scoped_array<char> buf(new char[buf_sz]);
    HANDLE infile = INVALID_HANDLE_VALUE;
    HANDLE outfile = INVALID_HANDLE_VALUE;
    BOOL success = FALSE;
 
    infile = ::CreateFileW(from_p, GENERIC_READ, FILE_SHARE_READ, NULL, 
    ...
    if (fail_if_exists)
    {
      HANDLE test = ::CreateFileW(to_p, GENERIC_READ, FILE_SHARE_READ, NULL,
                                  OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if (!::GetFileInformationByHandle(infile, &file_info))
    {
      ::CloseHandle(infile);
      return false;
    }
 
    outfile = ::CreateFileW(to_p, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
    ...
    while (success && ::ReadFile(infile, buf.get(), buf_sz, &bytes_read, NULL))
    {
      if (bytes_read == 0)
        break;
      ...
      while (total_written < bytes_read)
      {
        ...
        if (!::WriteFile(outfile, buf.get() + total_written, 
                         bytes_read - total_written, &chunk_written, NULL))
        {
          success = FALSE;
          break;
        }
    ...
    if (success)
    {
      ::SetFileTime(outfile, &file_info.ftCreationTime, 
                    &file_info.ftLastAccessTime, &file_info.ftLastWriteTime);
    }
...
#endif // RBX_PLATFORM_WIN_DURANGO

#if defined(RBX_PLATFORM_UWP)
  bool copy_file_api_uwp(const wchar_t* from_p, const wchar_t* to_p, bool fail_if_exists)
  {
    COPYFILE2_EXTENDED_PARAMETERS params = { sizeof(COPYFILE2_EXTENDED_PARAMETERS) };
    params.dwCopyFlags = fail_if_exists ? COPY_FILE_FAIL_IF_EXISTS : 0;
    HRESULT hr = CopyFile2(from_p, to_p, &params);
    return SUCCEEDED(hr);
  }
#endif
...
#  elif defined(RBX_PLATFORM_UWP)
    ...
    if (!::GetFileAttributesEx(p1.c_str(), GetFileExInfoStandard, &attr1))
    {
      error(true, p1, p2, ec, "boost::filesystem::equivalent");
      return false;
    }
    
    if (!::GetFileAttributesEx(p2.c_str(), GetFileExInfoStandard, &attr2))
    {
      error(true, p1, p2, ec, "boost::filesystem::equivalent");
      return false;
    }
    
    return attr1.nFileSizeHigh == attr2.nFileSizeHigh
...
#   elif defined(WINAPI_FAMILY_APP)  // UWP doesn't support DeviceIoControl
...
#   elif _WIN32_WINNT < 0x0600  // SDK earlier than Vista and Server 2008
    error(true, error_code(BOOST_ERROR_NOT_SUPPORTED, system_category()), p, ec,
          "boost::filesystem::read_symlink");
#   else  // Vista and Server 2008 SDK, or later (desktop only)
...
#else
      #ifndef RBX_DURANGO_TEMP_PATH
        #define RBX_DURANGO_TEMP_PATH L"T:\\Temp"
      #endif
      path p(RBX_DURANGO_TEMP_PATH);
      if (!is_directory(p))
      {
